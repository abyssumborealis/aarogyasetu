"""Generate reproducible synthetic hospital flow data with realistic correlations."""
...
RNG_SEED = 42
DEPARTMENTS = [
    {"hospital_id": "H001", "department": "CARDIOLOGY", "capacity": 22, "base_service": 11, "morning_peak": 1.6},
    {"hospital_id": "H001", "department": "ORTHOPEDICS", "capacity": 18, "base_service": 14, "morning_peak": 1.2},
    {"hospital_id": "H001", "department": "GENERAL", "capacity": 28, "base_service": 10, "morning_peak": 1.4},
    {"hospital_id": "H002", "department": "PEDIATRICS", "capacity": 16, "base_service": 12, "morning_peak": 1.3},
    {"hospital_id": "H002", "department": "DERMATOLOGY", "capacity": 14, "base_service": 15, "morning_peak": 0.9},
    {"hospital_id": "H002", "department": "ENT", "capacity": 15, "base_service": 13, "morning_peak": 1.1},
]
...
def generate_dataset(n_days: int = 21, start: datetime | None = None) -> dict[str, pd.DataFrame]:
    ...
    for day in range(n_days):
        ...
        for dept in DEPARTMENTS:
            doctors = 2 if dept["department"] in {"CARDIOLOGY", "GENERAL"} else 1
            avg_service = dept["base_service"] + rng.normal(0, 0.6)
            for hour in range(8, 19):
                ...
                for slot in range(4):
                    ...
                    # more registrations in first slots of peak hours
                ...
                arrival_rate = actual / 1.0
                service_rate = doctors * (60 / max(avg_service, 1))
                emergencies = int(rng.poisson(0.4 if dept["department"] != "CARDIOLOGY" else 0.8))
                people_ahead = max(current_queue - min(doctors, current_queue), 0)
                patients_being_served = min(doctors, current_queue)
                # correlated wait: more queue / fewer doctors / slower service => longer wait
                wait = (
                ...
                historical_avg = expected_hour * 0.9 + rng.normal(0, 1)
                crowd = (
                    current_queue
                ...
                crowd_rows.append(
                    {
                        ...
                        "historical_average_arrivals": max(historical_avg, 0),
